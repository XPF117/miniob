...
void SessionStage::handle_request(StageEvent *event)
{
  ...
  RC rc = communicator->write_result(sev, need_disconnect);  // 打印结果
...
{
  RC rc = query_cache_stage_.handle_request(sql_event);  // handle_request直接返回success
  if (OB_FAIL(rc)) {
  ...
  // 解析器阶段主要负责处理解析阶段生成的语法树或解析树，将其转化为内部表示形式，进行进一步的处理。
