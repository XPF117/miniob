...
// TODO:扩展使他们支持DATE
// 这段代码是一个函数，用于根据解析后的 SQL 语法节点（ParsedSqlNode）创建相应的 SQL 语句对象（Stmt 类的子类对象）。
// 具体来说，它根据传入的 ParsedSqlNode 的 flag 属性（表示 SQL 语句类型）选择相应的 Stmt 子类
// （如 InsertStmt、DeleteStmt、SelectStmt 等），并通过相应的 create 函数创建并返回该语句对象。
// 这个函数允许系统根据 SQL 语法的不同部分来创建相应的语句对象，从而方便后续的语句处理和执行。
